...
#include <stdlib.h>
#include <string.h>
#include <stdbool.h> //부울 자료형을 이용한 논리연산으로 수강신청을 세밀하게 작업하도록 기능을 추가함
        ...
        case '1':
            ...
            else
            {
                for (i = 0; i <= length; i++)
                {
                    ...
                    {
                        printf("\n\n 이미 수강신청을 한 학생입니다. \n\n");
                        goto not_insert; // 이미 신청한 학생을 배열에 추가할 필요가 없음(not_insert 레이블로 건너뜀)
                    }
                    if ((student[i].id == id))
                    {
                        printf("\n\n 학번과 이름이 서로 일치하지 않습니다. \n\n");
                        goto not_insert; //수강신청에 입력된 학생의 데이터와 틀림 단, 동명이인은 무시함
                    }
                }
                ...
                {
                    printf("\n\n 학번과 이름이 서로 일치하지 않습니다. \n\n");
                    goto not_insert; // 수강철회로 모인 학생의 데이터와 일치하지 않기 때문(not_insert 레이블로 건너뜀)
                }
                // 그 외의 경우는 노드에 학생 데이터가 없으므로 신규 학생을 의미함
                insert_list(student, &length, pos, id, name);
                printf("\n\n 요청하신 수강신청을 처리했습니다. \n\n");
            }
        not_insert: // goto문 사용이 권장되지는 않지만, for, if문이 중첩되면서 코드가 난잡해지는 단점이 더욱 크다고 판단되어 사용함
            break;
...
int search_node(std_node *first, int id, char *name)
{
    ...
    while (current != NULL)
    {
        n1 = (current->id == id);          // 학번이 같으면 1, 다르면 0
        n2 = !strcmp(current->name, name); // 이름이 같으면 1, 다르면 0

        if (n1 && n2) // 학번과 이름이 서로 일치할 경우(AND 연산)
        {
            return 1;
        }
        else if (n1 ^ n2) // 학번과 이름이 서로 일치하지 않음(XOR 연산)
        {
            return 2;
        }
        current = current->next;
